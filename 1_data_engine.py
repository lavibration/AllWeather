...
TICKERS = {
    ...
    "EU": {
        ...
        "Regimes": {
            ...
            "REFLATION": {"EXV1.DE": 0.3333, "EXV6.DE": 0.3333, "SXRS.DE": 0.3333},
...
TICKER_TO_NAME = {
    "XLK": "Tech US", "XLY": "Cons. Disc. US", "XLF": "Finance US", "XLE": "Energie US", "XLB": "Matériaux US",
    "XLRE": "Immobilier US", "XLC": "Comm. US", "XLU": "Utilities US", "DBC": "Commodities US",
    "EXV5.DE": "Energie EU", "EXV1.DE": "Banques EU", "EXV6.DE": "Ressources EU", "SXRS.DE": "Commodities EU",
    "EXV3.DE": "Tech EU", "EXV2.DE": "Automobile EU", "EXV9.DE": "Utilities EU", "EXI5.DE": "Immobilier EU",
    "EXV4.DE": "Santé EU", "EXV7.DE": "Télécoms EU", "GLD": "Or", "CASH": "CASH",
    "^GSPC": "S&P 500", "^STOXX50E": "Euro Stoxx 50"
}
...
def calculate_signals(data, zone):
    ...
    pos_map = TICKERS[zone]["Beta_Pos"] if zone == "US" else TICKERS[zone]["Beta_Pos"]
    neg_map = TICKERS[zone]["Beta_Neg"] if zone == "US" else TICKERS[zone]["Beta_Neg"]
    rets = data.pct_change()
    pos_rets = sum(rets[tk] * wt for tk, wt in pos_map.items()).fillna(0)
    neg_rets = sum(rets[tk] * wt for tk, wt in neg_map.items()).fillna(0)
...
if __name__ == "__main__":
    ...
    for z in ["US", "EU"]:
        ...
        hit_rate = len(r[r > 0]) / len(r[r != 0]) if len(r[r != 0]) > 0 else 0
